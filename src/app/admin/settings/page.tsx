...
export default function SettingsPage() {
  ...
  return (
    <div>
      ...
      >
        {settings.length === 0 ? (
          <p className="text-gray-500 text-sm text-center py-8">No settings configured yet. Add your first setting below.</p>
        ) : (
              ...
              <div key={setting.key} className="flex items-start gap-4 p-4 bg-gray-50 rounded-lg">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    <label className="text-sm font-semibold text-gray-900 font-mono">{setting.key}</label>
                    <span className="text-xs text-gray-400 bg-gray-200 px-1.5 py-0.5 rounded">{setting.type}</span>
                  </div>
                  {setting.type === "textarea" ? (
                    ...
                    <select
                      value={setting.value}
                      onChange={(e) => updateSettingValue(setting.key, e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[var(--color-burgundy)] focus:border-transparent outline-none"
                    >
        ...
        <div className="mt-6 pt-6 border-t border-gray-200">
          <h3 className="text-sm font-semibold text-gray-900 mb-3">Add New Setting</h3>
